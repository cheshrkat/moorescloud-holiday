!https://raw.githubusercontent.com/cheshrkat/holiday/master/holiday.png!
...
Since the official docs aren't online any more, this readme has some basic info on setting things up; and a copy of the PDF in case (like me) you lost your instruction book.

I've tried to write this without assuming knowledge, as I know a few people use their Moorescloud Holiday to try out coding with their kids. If you know things like colour formats, just skip those sections.
...
* OSX Terminal on Mac
* Windows Subsystem for Linux (WSL) on Windows; or Cygwin. Note the default Command Prompt does not work for this!
...
* "Holiday manual (PDF)":https://raw.githubusercontent.com/cheshrkat/holiday/master/holiday-manual.pdf covers finding and connecting to your Holiday. Note you can also find via MAC address in your router, the MAC's printed on the Holiday unit.
* "Holiday hardware architecture (PDF)":https://raw.githubusercontent.com/cheshrkat/holiday/master/holiday-architecture-rev2.pdf has deeper hardware info that you probably don't really need.  
...
## @ssh holiday@yourholidaysname@
## Default password is @holiday@
## Once you've logged in you should see something like @[holiday@yourholidaysname ~]$@
# It is recommended you change the default password by running the command @passwd@ on the Holiday; but be sure to put that new password into your password safe for future reference.

There are two Holiday APIs:

# "The UDP 'Secret API'":https://github.com/moorescloud/secretapi - see the github repo for documentation.
...
* @/iotas/0.1/device/moorescloud.holiday/localhost/setlights@ - sends a frame of colours to set the lights
* @/iotas/0.1/device/moorescloud.holiday/localhost/gradient@ - starts a gradient transition for the lights
...
...and just in case you're now sitting with your Holiday on your desk blinding you, here's how to set them all off:
...
*Gradient* takes a JSON object with @begin@, @end@ and @steps@ defining the start and end colours in RGB arrays; and the number of steps to take in the transition. For example to start from black and work up to full white in 50 steps:
...
To finish a gradient with the lights entirely off, steps must be set to at least half the highest value. Remember to count zero in th 0-255 range, so there are 256 steps; which means to go from full brightness to black, you must use at least 128 steps (see example above).

h3. Trying the APIs in bash

A simple way to try this out is to create @foo.json@ files and send them to your Holiday in bash using @curl@. You can give this a go with @on.json@ and @off.json@ in the @/bash/@ directory, or you can create your own files.
...
# @gradient-on.json@ with the gradient example above
...
* Ensure you have @curl@ by running @type curl@ in your bash shell.
...
!https://raw.githubusercontent.com/cheshrkat/holiday/master/holiday-visualiser.png!

It's effectively an alternative UI to control the Holiday. It wasn't really what I intended to do but that's how it turned out :)

This one's a little different as *you have to put the code onto the holiday* (to avoid CORS issues) to actually control the lights. 

If you view it locally (by opening @javascript/index.html@ in a browser) it still does the visualisation but it won't actually affect the lights. Not actually bad to practice before uploading your code to the Holiday.

Setup:

# Enable SSH on your holiday as noted earlier
## Open Terminal/bash shell in your local copy of @/javascript/@ and use <code>scp * holiday@yourholidaysname:/home/holiday/holideck/iotas/www/apps/test/</code> to copy the files to your holiday.
## Copy the files in @/javascript/@ to @/home/holiday/holideck/iotas/www/apps/test/@ using sFTP
